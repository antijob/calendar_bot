...
class TestFilterEvents:
    ...
    def test_filter_events_tomorrow(self):
        ...
        today, week, two_week = filter_events(
            events, reference_time=self.get_base_time())
        assert len(today) == 1
    ...
    def test_filter_events_multiple_same_day(self):
        ...
        events = [
            self.create_event("Event 1 tomorrow",
                              days_offset=1, hours_offset=0),
            self.create_event("Event 2 tomorrow",
                              days_offset=1, hours_offset=5),
        ]
        today, week, two_week = filter_events(
            events, reference_time=self.get_base_time())
        assert len(today) == 2
...
class TestEnvironmentValidation:
    ...
    def test_required_vars_in_env_example(self):
        ...
        required = ["GOOGLE_API_KEY", "CALENDAR_ID",
